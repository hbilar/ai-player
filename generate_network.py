...
def mutate_net(net, max_jitter, mutation_rate):
    ...
    for n in range(0, len(new_net['layers'])):
        ...
        for w1 in layer['weights']:
            ...
            for w2_idx in range(0, len(w1)):
                ...
                if r < mutation_rate:
                    w1[w2_idx] += random.uniform(-max_jitter, max_jitter)

        # Add jitter to the biases
        for b_idx in range(0, len(layer['bias'])):
            r = random.uniform(0, 1)
            if r < mutation_rate:
                layer['bias'][b_idx] += random.uniform(-max_jitter, max_jitter)
...
def handle_cmdline(input, weight_max_jitter, mutation_likelihood, output):
    ...
    new_net = mutate_net(input_net, weight_max_jitter, mutation_likelihood)
